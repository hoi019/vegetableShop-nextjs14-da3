...
const ProductDetail = () => {   
   ...
   const handleShowProductByCate = async (categoryId: number) => {
      try {
         const response = await fetch(`http://localhost:8000/api/product/${categoryId}/bycate`)
         if (response.ok) {
   ...
   const handlePrev = () => {
      setCurrentIndex((prevIndex) => (prevIndex === 0 ? productRelated.length - 1 : prevIndex - 1))
   }

   const handleNext = () => {
      setCurrentIndex((prevIndex) => (prevIndex === productRelated.length - 1 ? 0 : prevIndex + 1))
   }
